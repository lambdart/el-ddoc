...
(defcustom dash-docs-enable-debugging nil
...
(defun dash-docs--install-docset (url docset-name)
  ...
  (let ((docset-tmp-path
         (format "%s%s-docset.tgz" temporary-file-directory docset-name)))
...
(defun dash-docs-read-json-from-url (url)
  ...
  (let ((buffer (url-retrieve-synchronously
    ...
    (if (not buffer)
        (error "Was not possible to retrieve url: %s content" url))
...
The first element is the docset's name second the docset's archive url."
  (let ((user-docs (dash-docs-read-json-from-url dash-docs-unofficial-url)))
...
(defun dash-docs-install-docset (docset-name)
  ...
  (when (dash-docs--ensure-created-docsets-path (dash-docs-docsets-path))
    (let ((feed-url (format "%s/%s.xml" dash-docs-docsets-feed-url docset-name))
...
;; ;;;###autoload
;; (defun dash-docs-async-install-docset (docset)
;;   "Asynchronously download DOCSET."
